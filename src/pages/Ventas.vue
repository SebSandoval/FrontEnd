                        ...
                        ><p
                          :class="{
                            'caption font-weight-black mx-auto mt-3':
                              $vuetify.breakpoint.xs,
                            'mx-auto mt-3 font-weight-black':
                  ...
                  <v-card>
                    ...
                    <v-card-text>
                      ...
                      >
                        ...
                        <template
                          v-if="textCantidad === true"
                          v-slot:[`item.descuentoProducto`]="{ item }"
                        >
                          <v-text-field
                            type="number"
                            v-model="item.descuentoProducto"
                            @keyup="onChange(item)"
                          >
                          </v-text-field>
                        </template>
...
export default {
  data: () => ({
    ...
    encabezadoDescripcion: [
      ...
      { text: "Descuento en %", value: "descuentoProducto", align: "center" },
    ...
    encabezadoArticulos: [
      ...
      { text: "Nombre", value: "nombre", class: "subtitle font-weight-black" },
      { text: "Stock", value: "stock", class: "subtitle font-weight-black" },
      {
        text: "Precio",
        value: "precioVenta",
  ...
  computed: {
    ...
    totalVenta() {
      ...
      this.productosDescripcion.map((x) => (this.totalV += x.subtotal));

      return this.totalV;
    },
    subTot() {
      return this.totalV - this.imp;
    },
      ...
      if (this.searchVenta===""){
        return this.desserts
         ...
         return this.desserts.filter(x=>{
           return (x.numeroComprobante).toString().indexOf(this.searchVenta)!=-1
         })
  ...
  methods: {
    ...
    traer() {
      ...
      axios
        .get("venta", header)
        .then((response) => {
          this.desserts = response.data.venta;
    ...
    this.imp=  this.productosDescripcion.reduce((x, y)  => x+=(y.subtotal* this.editedItem.impuesto), 0 ) 
    ...
    onChange(item) {
      item.subtotal =
        ...
        (item.cantidadProducto * item.precioProducto * item.descuentoProducto) /
          100;
     this.imp=  this.productosDescripcion.reduce((x, y)  => x+=(y.subtotal* this.editedItem.impuesto), 0 ) 
    ...
    infoItem(item) {
      item.detalles.map((x) => {
        this.productosDescripcion.push({
          ...
          descuentoProducto: x.descuentoProducto,
          subtotal:
            x.cantidadProducto * x.precioProducto -
            (x.cantidadProducto * x.precioProducto * x.descuentoProducto) / 100,
        });
      });
    ...
    ActivarDesactivar(item) {
      ...
      this.estado === 1
        ? (this.dialogText = "¿Seguro que desea desactivar esta venta?")
        : (this.dialogText = "¿Seguro que desea activar esta venta?");
    },
